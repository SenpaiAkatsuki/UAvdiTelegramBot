...
def build_application_vote_text(
    ...
    branch_label = {
        "matched": "з бота",
        "unlinked": "із сайту (без Telegram)",
    }.get(branch, branch)
    return (
        f"🗳 Нова заявка на розгляд ({branch_label})\n\n"
        f"📄 ID заявки #{application.get('id')}\n"
        f"👤 ПІБ: {application.get('applicant_name') or '-'}\n"
        f"📞 Телефон: {application.get('contact_phone') or '-'}\n"
        f"✉️ Email: {application.get('contact_email') or '-'}\n"
        f"🩺 Спеціалізація: {application.get('specialization') or '-'}\n"
        f"📎 Документ: {application.get('document_file_name') or '-'}\n\n"
...
async def notify_vote_result_user(
    ...
    if approved:
        if application_row.get("status") == "APPROVED_AWAITING_PAYMENT":
            await notify_user(
                ...
                user_id=tg_user_id,
                text=(
                    "✅ Вашу заявку підтверджено голосуванням спільноти.\n"
                    "Тепер можна переходити до оплати."
                ),
        ...
        await notify_user(
            bot=bot,
            user_id=tg_user_id,
            text="✅ Контент заявки підтверджено. Очікуйте наступного кроку.",
            context={
    ...
    await notify_user(
        ...
        text=(
            ...
            "За деталями зверніться до адміністраторів."
        ),
        context={
